...
//!
//! Function registration and catalog metadata live here (`build_worker`) so both
//! entrypoints share them verbatim: `main.rs` (the native binary, stdio/HTTP
//! transport) and the `code-wasm` crate (the browser build, which serves the
//! same `Worker` over a SharedArrayBuffer byte channel instead).
...
/// catalog metadata. Shared verbatim by the native binary (`main.rs`) and the
/// browser build (`code-wasm`); only the transport differs (`run` vs the SAB
/// byte-channel serve). Does NOT call `.run()`.
