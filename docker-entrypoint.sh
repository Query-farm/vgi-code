...
#
# Dispatch the single vgi-code image into one of its transports:
...
#
# The worker is stateless (tree-sitter grammars compiled into the binary), so
...
case "${1:-http}" in
  http)
    ...
    exec code-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec code-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec code-worker "$@"
