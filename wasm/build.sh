#!/usr/bin/env bash
# Build the browser (`worker:`) build of the code VGI worker: code-wasm compiled
# to wasm32-unknown-emscripten, linked by emcc into a MODULARIZE'd Web Worker
# module served over a SharedArrayBuffer byte channel.
#
...
#   - a nightly toolchain with rust-src: -Z build-std is required because the
...
# and without them wasm-ld rejects --shared-memory. --no-entry guards against a
# transitive dependency declaring a cdylib crate-type that emcc would otherwise
# try to link as a program.
#
# CFLAGS_<target>: the tree-sitter crates ship C (tree-sitter core `lib.c` plus
# each grammar's `parser.c`/`scanner.c`) compiled by the `cc` crate — NOT rustc —
# so the RUSTFLAGS target-features above never reach them. Without -pthread those
# C objects are built with neither `atomics` nor `bulk-memory`, and wasm-ld then
# rejects the whole link ("--shared-memory is disallowed by …-lib.o"). -pthread
# makes emcc emit atomics+bulk-memory for the C, matching the Rust objects.
export CFLAGS_wasm32_unknown_emscripten="-pthread -matomics -mbulk-memory${EXTRA_CFLAGS:-}"
...
LIB="target/$TARGET/release/libcode_wasm.a"
...
#
# OPT_LEVEL=3 overrides the workspace's release opt-level for this target only;
# it does not change the native build.
#
# MALLOC=mimalloc is load-bearing for multithreaded serves: emscripten's default
# dlmalloc serializes every allocation on a global lock, so concurrent serve
# threads convoy on it; mimalloc is thread-caching. Override with MALLOC=dlmalloc.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten's 64 KiB default overflows
# (the Arrow encoders nest deeply); an overflow surfaces as a bare "memory access
# out of bounds" with no stack trace. INITIAL_MEMORY must cover main stack + every
# pthread stack up front so startup does not stall growing shared memory.
echo "==> emcc link"
